...
extension VideoViewController: UITableViewDelegate, UITableViewDataSource, UITableViewDataSourcePrefetching {
    ...
    func tableView(_ tableView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
        for indexPath in indexPaths {
            ...
            if video.documents.count - 1 == indexPath.row && page < 15 && !isEnd {
                page += 1
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let video else { return 1 }
        print(video.documents.count)
        return video.documents.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let video = video {
            let row = video.documents[indexPath.row]
            cell.titleLabel.text = row.title
            cell.detailLabel.text = "\(row.author) | \(row.playTime)초"
